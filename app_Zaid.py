...
# Authenticate with Google Sheets
    ...
    sheet1_url = "https://docs.google.com/spreadsheets/d/1WIIsfz5edYOslsMtIbyNngW1S6obS6DcQ4JRzTHcXbs/edit?gid=1127150915#gid=1127150915" # Jang
    sheet2_url = "https://docs.google.com/spreadsheets/d/1awWAuJcxWTyC0qZSvQ06s0JiDUQ7MX7LwTHCTCgxYic/edit?gid=0#gid=0" # Lifestyle 
    sheet1 = client.open_by_url(sheet1_url).worksheet("The News")
    ...
    @app.route("/zaid/keywords/<date>")
    def get_primary_secondary_zaid(date):
        try:
    ...
    @app.route("/zaid/strategy_data/<date>/<keyword>/<type>")
    def get_strategy_data_zaid(date, keyword, type):
        try:
            df = load_strategy_data()
            target_date = pd.to_datetime(normalize_date_string(date)).date()
    ...
    @app.route("/zaid/matching_keywords", methods=["POST"])
    def keyword_matching_zaid():
        try:
    ...
    @app.route("/zaid/get_data", methods=["POST"])
    def get_data_zaid():
        try:
            data = request.get_json()
            selected_date = normalize_date_string(data['date'])
    ...
    # --- Data Table API 
    @app.route("/zaid/get_table_data", methods=["POST"])
    def get_table_data_zaid():
        try:
            data = request.get_json()
    ...
    @app.route("/zaid")
    def home_zaid():
        return render_template("home_zaid.html")
